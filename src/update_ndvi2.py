...
def main():
    ...
    with app.app_context():
        ...
        # NO MOVER LOS ARCHIVOS TODAVÍA - los necesitamos para procesar la BBDD
        # Los moveremos DESPUÉS de procesar todos los recintos
        ...
        try:
            ...
            # 2) UPSERT batch en indices_raster (SIN id_parcela)
            #    Requiere UNIQUE (id_imagen, id_recinto, tipo_indice)
            sql_idx = text("""
            ...
            # USAR SIEMPRE EL ARCHIVO TEMPORAL RECIÉN CREADO
            tif_to_open = tif_path_utm
                ...
                for id_recinto, gj, srid in recintos:
                    ...
                    try:
                        ...
                        if DEBUG_S2 and (processed % DB_PROGRESS_EVERY == 0):
                            ...
                            print(f"[BBDD] procesados={processed} | con_stats={inserted} | fuera_bounds={out_of_bounds} | sin_pixels={no_valid_pixels} | errores={errors} | {elapsed:.1f}s")
                    
                    except Exception as e:
                        errors += 1
                        if DEBUG_S2:
                            print(f"[BBDD] Error procesando recinto {id_recinto}: {e}")
                        continue

            # Fuera del with, insertar los últimos
            if rows_to_insert:
                db.session.execute(sql_idx, rows_to_insert)
            ...
            print(f"\n[BBDD] ===== RESUMEN FINAL =====")
            print(f"  Total recintos en DB: {len(recintos)}")
            print(f"  Procesados: {processed}")
            print(f"  Insertados (con stats): {inserted}")
            print(f"  Fuera de bounds: {out_of_bounds}")
            print(f"  Sin píxeles válidos: {no_valid_pixels}")
            print(f"  Errores: {errors}")
            print(f"  Diferencia total: {len(recintos) - inserted}")
            print(f"OK: NDVI guardado en BBDD -> imagenes.id_imagen={id_imagen} | indices_raster upserts={inserted}")
            
            # -----------------------------
            # AHORA SÍ: Mover archivos a latest (después de procesar BBDD)
            # -----------------------------
            print("\n[NDVI] Moviendo archivos temporales a latest...")
            ...
            if not ok_tif_utm:
                ...
                print(f"OK: {tif_path_utm.name} -> {latest_tif_utm.name}")
            if not ok_tif_3857:
                print("WARNING: no pude reemplazar ndvi_latest_3857.tif (bloqueado).")
            else:
                print(f"OK: {tif_path_3857.name} -> {latest_tif_3857.name}")
            if not ok_png_3857:
                print("WARNING: no pude reemplazar ndvi_latest_3857.png (bloqueado).")
            else:
                print(f"OK: {png_path_3857.name} -> {latest_png_3857.name}")
