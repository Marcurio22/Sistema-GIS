...
DAYS_BACK = int(os.getenv("S2_DAYS_BACK", "400"))
...
def main():
    ...
    with app.app_context():
        ...
        date_range_str = ""
        ndvi_date = now  # Por defecto fecha de procesamiento
        
        if image_dates:
            min_date = min(image_dates)
            max_date = max(image_dates)
            ndvi_date = max_date  # USAR LA FECHA DE LA IMAGEN MÁS RECIENTE
        ...
        meta = {
            ...
            "ndvi_date": ndvi_date.isoformat(),  # FECHA DE LAS IMÁGENES
            "ndvi_date_formatted": ndvi_date.strftime("%Y%m%d"),  # FORMATO PARA NOMBRES DE ARCHIVO
        ...
        if date_range_str:
            ...
            print(f"[INFO] Fecha NDVI para archivos: {ndvi_date.strftime('%Y%m%d')}")
        ...
        try:
            # Leer la fecha del NDVI (de las imágenes, no del script)
            ...
            sql_img = text("""
                ...
                RETURNING id_imagen
            """)
            ruta_rel = str(Path("static") / "ndvi" / "ndvi2_latest_utm.tif")
            id_imagen = db.session.execute(sql_img, {
                "origen": "satelite",
                "fecha": ndvi_date,  # Fecha de las imágenes satelitales
                "epsg": int(dst_crs.to_epsg() or 0),
                "sensor": f"Sentinel-2 L2A (PC) NDVI2 | {NDVI_COMPOSITE} | {date_range_str}",
                "res": float(NDVI_RES_M),
                "minx": minx2, "miny": miny2, "maxx": maxx2, "maxy": maxy2,
                "ruta": ruta_rel,
            }).scalar()
            
            print(f"[BBDD] Imagen insertada con id_imagen={id_imagen}, fecha={fecha_str}")
            ...
            tif_to_open = latest_tif_utm if ok_tif_utm else tif_path_utm
            ...
            with rasterio.open(str(tif_to_open)) as ds:
                ...
                for id_recinto, gj, srid in recintos:
                    ...
                    try:
                        ...
                        # Ruta del thumbnail: {fecha}_{id_recinto}.png
                        ruta_thumbnail = f"webapp/static/thumbnails/{fecha_str}_{id_recinto}.png"
                        
                        rows_to_insert.append({
                            "id_imagen": int(id_imagen),
                            "id_recinto": int(id_recinto),
                            "tipo": "NDVI",
                            "fecha_calc": now,           # Cuando se ejecutó el script
                            "fecha_ndvi": ndvi_date,     # Fecha de las imágenes satelitales
                            "epsg": int(dst_crs.to_epsg() or 0),
                            "res": float(NDVI_RES_M),
                            "mean": stats["mean"],
                            "min": stats["min"],
            ...
            print(f"✓ NDVI2 guardado en BD (id_imagen={id_imagen}, fecha={fecha_str})")
