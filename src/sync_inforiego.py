...
def parse_fecha(fecha_val):
    ...
    if isinstance(fecha_val, (int, float)):
        try:
            # ✅ Fix: usar zona horaria de Madrid en lugar de UTC
            # Con utcfromtimestamp, un timestamp de medianoche en España
            # se convertía al día anterior (ej: 7 marzo → 6 marzo a las 23:00 UTC)
            return datetime.fromtimestamp(fecha_val / 1000, tz=TZ_MADRID).date()
        except Exception:
            return None
    try:
